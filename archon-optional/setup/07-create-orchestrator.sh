...
# 1. 检查 Python 编排器
echo "## 1. Python 编排器"
if [ -f "$ARCHON_DIR/tools/orchestrator.py" ]; then
    echo -e "${GREEN}✓${NC} orchestrator.py 已创建"
    chmod +x $ARCHON_DIR/tools/orchestrator.py
else
    echo -e "${YELLOW}!${NC} orchestrator.py 未找到"
fi
echo ""

# 2. 创建简化的 Shell 编排器
cat > $ARCHON_DIR/tools/archon.sh << 'EOF'
#!/bin/bash
# Archon 系统主入口脚本
...
chmod +x $ARCHON_DIR/tools/archon.sh

echo -e "${GREEN}✓${NC} Shell 编排器: $ARCHON_DIR/tools/archon.sh"
echo ""
...
echo "  sudo ln -s $ARCHON_DIR/tools/archon.sh /usr/local/bin/archon"
echo ""

# 4. 测试编排器
echo "## 3. 测试编排器"
$ARCHON_DIR/tools/archon.sh help
echo ""

echo -e "${GREEN}✓ 编排器创建完成${NC}"
echo ""
echo "使用方法:"
echo "  $ARCHON_DIR/tools/archon.sh help"
echo ""
echo "下一步:"
echo "  bash ~/ai/workspace/archon/setup/08-final-setup.sh"
